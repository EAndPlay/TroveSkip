...
namespace TroveSkip.ViewModels
{
    public sealed partial class MainWindowViewModel
    {
        ...
        private static int[] SettingsPointerSignature =
        {
        ...
        private static int[] ChatStateOffsetSignature =
        {
        ...
        private static int[] MarketMasteryCheckSignature =
        {
        ...
        private static readonly Dictionary<Instructions, ASMInstruction[]> _instructions = new()
        {
            {
        ...
        private static readonly int[] ZoomHackSignature = {0xF3, 0x0F, 0x11, 0x5F, 0x2C};

        private static readonly int[] ZoomHackEnabledSignature = {0xF3, 0x0F, 0x11, 0x57, 0x2C};

        private static readonly int[] FovHackSignature =
            {0xF3, 0x0F, 0x10, 0x41, 0x28, 0xF3, 0x0F, 0x59, 0x05, -1, -1, -1, -1, 0x8B, 0x4D, 0xF4};

        private static readonly int[] FovHackEnabledSignature =
            {0xF3, 0x0F, 0x10, 0x41, 0x28, 0xF3, 0x0F, 0x59, 0x0D, -1, -1, -1, -1, 0x8B, 0x4D, 0xF4};

        private static readonly int[] AntiAfkSignature = {0x55, 0x8B, 0xEC, 0x83, 0xE4, 0xF8, 0xD9, 0xEE, 0x83, 0xEC, 0x58, 0x56, 0x8B, 0xF1, 0x57, 0xDD, 0x46, 0x20, 0xDF, 0xF1};
        private static readonly byte[] AntiAfkCaveSignature = {0xC3, 0x55, 0x8B, 0xEC, 0x83, 0xE4, 0xF8};

        private static readonly int[] MapHackSignature =
        {
            0x77, -1, 0xB8, -1, -1, -1, -1, 0xF3, 0x0F, 0x10, 0x08, 0xF3, 0x0F, 0x11, 0x89, -1, -1, -1, -1, 0x8B, 0x89,
            0xA8, 0x00, 0x00, 0x00, 0x85, 0xC9, 0x74, -1
        };

        private static readonly int[] MapHackEnabledSignature =
        {
            0xEB, -1, 0xB8, -1, -1, -1, -1, 0xF3, 0x0F, 0x10, 0x08, 0xF3, 0x0F, 0x11, 0x89, -1, -1, -1, -1,
            0x8B, 0x89, 0xA8, 0x00, 0x00, 0x00, 0x85, 0xC9, 0x74, -1
        };

        private static readonly int[] GeodeToolSignature = {0xDF, 0xF1, 0xDD, 0xD8, 0x72, 0x35, 0x8D, 0x64, 0x24};

        private static readonly int[] GeodeToolEnabledSignature = {0xDF, 0xF0, 0xDD, 0xD8, 0x72, 0x35, 0x8D, 0x64, 0x24};

        private static readonly int[] MiningSlowSignature = {0xDF, 0xF1, 0xDD, 0xD8, 0x72, 0x61};

        private static readonly int[] MiningSlowEnabledSignature = {0xDF, 0xF0, 0xDD, 0xD8, 0x72, 0x61};
        ...
        private static readonly int[] ChamsMonsters =
        {
            0x0F, 0x29, 0x07, 0x8B, 0xC7, 0x5F, 0x5E, 0x8B, 0xE5, 0x5D, 0x8B
        };

        private static readonly int[] ChamsMonstersEnabled =
        {
            0x0F, 0x29, 0x0F, 0x8B, 0xC7, 0x5F, 0x5E, 0x8B, 0xE5, 0x5D, 0x8B
        };

        private static readonly int[] NoClip =
        {
            0x0F, 0x84, 0xF0, 0x05, 0x00, 0x00, 0x0F, 0x28, 0x45, 0xE0, 0x0F, 0x28, 0x65, 0xD0
        };

        private static readonly int[] NoClipEnabled =
        {
            0x0F, 0x87, 0xF0, 0x05, 0x00, 0x00, 0x0F, 0x28, 0x45, 0xE0, 0x0F, 0x28, 0x65, 0xD0
        };
        
        private static readonly int[] AutoLootSignature =
        {
            0xDD, 0x87, 0x78, 0x03, 0x00, 0x00, 0xDF, 0xF1, 0xDD, 0xD8, 0x76, 0x45, 0xDD, 0x87, 0x78, 0x03, 0x00, 0x00
        };
        
        private static readonly int[] AutoLootEnabledSignature =
        {
            0xDD, 0x87, 0x78, 0x03, 0x00, 0x00, 0xDF, 0xF1, 0xDD, 0xD8, 0xEB, 0x27, 0xDD, 0x87, 0x78, 0x03, 0x00, 0x00
        };

        private static readonly int[] AntiDismountSignature = {0x74, 0x0B, 0x8B, 0x07, 0x8B, 0xCF, 0x6A, 0x00, 0x6A};

        private static readonly int[] AntiDismountEnabledSignature = {0xEB, 0x0B, 0x8B, 0x07, 0x8B, 0xCF, 0x6A, 0x00, 0x6A};
        
        private static readonly int[] AutoAttackSignature = {0xDF, 0xF1, 0xDD, 0xD8, 0x72, 0x1F};

        private static readonly int[] AutoAttackEnabledSignature = {0xDF, 0xF0, 0xDD, 0xD8, 0x72, 0x1F};
        ...
        private static int GetCursorPositionLParam(int x, int y)
        {
            return (y << 16) | (x & 0xFFFF);
        }

        private static SystemMessage GetSystemMessage(MouseButton mouseButton, bool keyDown) => mouseButton switch
        {
        ...
        private unsafe string ReadStringToEnd(IntPtr handle, int address, Encoding encoding,
        ...
        private void WriteIntToLocalPlayer(int[] offsets, int value) =>
            WriteMemory(GetAddressFromLocalPlayer(offsets), BitConverter.GetBytes(value));

        private void WriteUIntToLocalPlayer(int[] offsets, uint value) =>
            WriteMemory(GetAddressFromLocalPlayer(offsets), BitConverter.GetBytes((int) value));

        private void WriteFloatToLocalPlayer(int[] offsets, float value) =>
            WriteMemory(GetAddressFromLocalPlayer(offsets), BitConverter.GetBytes(value));

        private void WriteFloatToLocalPlayer(IntPtr handle, int[] offsets, float value) => WriteMemory(handle,
            GetAddressFromLocalPlayer(handle, offsets), BitConverter.GetBytes(value));
        ...
        private int FindSignatureAddress(int[] pattern) => FindSignatureAddress(pattern, HookModel ?? Hooks[0]);
        
        private unsafe int FindSignatureAddress(int[] pattern, HookModel hook = null)
        {
            hook ??= HookModel ?? Hooks[0];
        ...
        private int InvokeFunction(HookModel hook, uint functionAddress, IntPtr param)
        {
        ...
        private bool ReadMemory(int address, byte[] buffer) =>
            ReadProcessMemory(_handle, address, buffer, buffer.Length, out _);

        private bool ReadMemory(IntPtr handle, int address, byte[] buffer) =>
            ReadProcessMemory(handle, address, buffer, buffer.Length, out _);

        private unsafe bool ReadMemory(int address, byte* buffer, int size = 4) =>
            ReadProcessMemory(_handle, address, buffer, size, out _);

        private unsafe bool ReadMemory(IntPtr handle, int address, byte* buffer, int size = 4) =>
            ReadProcessMemory(handle, address, buffer, size, out _);

        private void WriteMemory(int address, byte[] buffer) =>
            WriteProcessMemory(_handle, address, buffer, buffer.Length, out _);

        private void WriteMemory(int address, byte[] buffer, int size) =>
            WriteProcessMemory(_handle, address, buffer, size, out _);

        private void WriteMemory(IntPtr handle, int address, byte[] buffer) =>
            WriteProcessMemory(handle, address, buffer, buffer.Length, out _);

        private void WriteMemory(IntPtr handle, int address, byte[] buffer, int size) =>
            WriteProcessMemory(handle, address, buffer, size, out _);

        private int AllocateMemory(IntPtr handle, int size, MemoryProtection protection = MemoryProtection.ExecuteRead,
        ...
        private int GetAddressFromLocalPlayer(IntPtr handle, int[] offsets) =>
            GetAddress(handle, _currentLocalPlayerPointer, offsets);
        ...
        [DllImport("kernel32.dll")]
        private static extern IntPtr CreateRemoteThread(IntPtr handle,
        ...
        private static extern IntPtr SetTimer(IntPtr windowHandle, IntPtr nIDEvent, uint uElapse,
        ...
        [DllImport("user32.dll")]
        static extern IntPtr SetCapture(IntPtr windowHandle);
